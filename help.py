...
for x, y in [("iflag", "input mode"),
    ...
    print(f"    {' '.join(sorted(stty.all_attributes['boolean'][x]))}\n")

print("  Winsize attributes (possible values: any nonnegative integer):")
print(f"    {' '.join(sorted(stty.all_attributes['winsize']))}\n")

print("  Non-canonical mode-related attributes (possible values: any nonnegative integer):")
print(f"    {' '.join(sorted(stty.all_attributes['non_canonical']))}\n")

print("  CSIZE and *DLY attributes:")
heading1 = "ATTRIBUTE"
heading2 = "POSSIBLE VALUES"
csize_key = "csize"
csize_values = ", ".join(sorted([f'stty.{v}, "{v}"' for v in stty.all_attributes["csize"]]))

padding = max(len(x) for x in stty.all_attributes["delay_masks"])
...
# Print the *DLY rows.
for mask, maskvalset in stty.all_attributes["delay_masks"].items():
    mask_values = ", ".join(sorted([f'stty.{v}, "{v}"' for v in maskvalset]))
    print(f"    {mask:^{padding}}  |  {mask_values}")

print("\n  Control character attributes:")
print(f"    ATTRIBUTES: {' '.join(sorted(stty.all_attributes['control_character']))}")
print(control_character_value_help)

print("  Speed attributes:")
print(f"    ATTRIBUTES: {' '.join(sorted(stty.all_attributes['speed']))}")
print(f"    POSSIBLE VALUES: {', '.join([str(n) for n in sorted(stty.all_attributes['baud_rates'])])}")
